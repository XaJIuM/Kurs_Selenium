...
try:                                                             
    ...
    browser.execute_script("return arguments[0].scrollIntoView(true);", input3)  # Передаем аргумент JavaScript и скроллим кнопку радиокнопку "Robots rule"
    input3.click()                                                               # Кликаем по радиокнопке "Robots rule"
    time.sleep(2)                                                                # Устанавливаем задержку 2 сек
    
    button = browser.find_element(By.XPATH, '//button[@class="btn btn-primary"]')             # Находим кнопку "Submit"
